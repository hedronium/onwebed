...
},{"./bundle-url":"../../../../.nvm/versions/node/v11.0.0/lib/node_modules/parcel-bundler/src/builtins/bundle-url.js"}],"../sass/edit.sass":[function(require,module,exports) {
var reloadCSS = require('_css_loader');

module.hot.dispose(reloadCSS);
module.hot.accept(reloadCSS);
},{"_css_loader":"../../../../.nvm/versions/node/v11.0.0/lib/node_modules/parcel-bundler/src/builtins/css-loader.js"}],"../../document_editor/src/Main.elm":[function(require,module,exports) {
(function(scope){
'use strict';
	...
	function (id, label, content, parent, type_) {
		var labelElements = function () {
			if (label.$ === 'Nothing') {
				...
				var label_ = label.a;
				return author$project$LabelProcessor$processLabel(label_);
			}
		}();
		return author$project$Types$Box(
			{content: content, id: id, label: label, labelElements: labelElements, parent: parent, type_: type_});
	});
...
var author$project$Box$jsonStringToDocument = function (jsonString) {
	...
	var boxContentDecoder = A2(
		...
		A2(
			elm$json$Json$Decode$andThen,
			function (content) {
				return (!elm$core$String$length(content)) ? elm$json$Json$Decode$succeed(elm$core$Maybe$Nothing) : elm$json$Json$Decode$succeed(
			...
			if (maybeFirstOldBox.$ === 'Nothing') {
				...
				if (maybeFirstNewBox.$ === 'Nothing') {
					...
					var updatedContentAndLabel = A2(
						...
						function (_n2) {
							var box = _n2.a;
							return (_Utils_eq(box.id, newFirstNewBoxId) && _Utils_eq(box.type_, author$project$Types$LiquidBox)) ? author$project$Types$Box(
								_Utils_update(
									box,
									{
									...
									})) : ((_Utils_eq(box.id, newFirstNewBoxId) && _Utils_eq(box.type_, author$project$Types$SolidBox)) ? author$project$Types$Box(
								_Utils_update(
									box,
									{
										label: firstOldBox.label,
...
var author$project$Box$removeBoxes = F2(
	function (boxesIds, model) {
		removeBoxes:
		while (true) {
...
var author$project$Odl$boxToOdl = F3(
	function (model, level, _n0) {
		...
		var closing_tag = _Utils_eq(box.type_, author$project$Types$SolidBox) ? ((!elm$core$String$length(content)) ? '!>' : (indentation + '!>')) : ' !]';
		return _Utils_ap(
			opening_tag,
			_Utils_ap(
				label,
...
var author$project$Odl$odlStringOfDocument = function (model) {
		...
		switch (msg.$) {
			...
			case 'ApplyOdlInsideBox':
				...
				var newDocument = _Utils_ap(
					A2(author$project$Box$removeBoxes, childrenIds, newModel),
					boxesWithUpdatedParents);
				var newModel2 = _Utils_update(
					newModel,
					{document: newDocument, documentDraft: _List_Nil, selectedBoxId: 0, status: author$project$Types$Default});
				return _Utils_Tuple2(
					newModel2,
					author$project$State$overlay(false));
			default:
				...
				var command = function () {
					switch (machine_name) {
						case 'view_odl':
							var odlString = author$project$Odl$odlStringOfDocument(model);
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text('You have an unsaved draft. Apply the changes or press escape again to discard the changes.')
							]))
					])) : A2(
				elm$html$Html$div,
				_List_fromArray(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text('Press Escape to go back, but any change you make will be discarded unless you apply it.')
							]))
					])),
				A2(
				elm$html$Html$div,
				...
				_List_fromArray(
					[
						elm$html$Html$text('Apply Changes')
					]))
			]);
	});
		...
		[
			...
			_List_fromArray(
				[
					...
					elm$html$Html$Attributes$class('button is-outlined')
				]),
			_List_fromArray(
				[
					A2(
																		...
																		if (_Utils_eq(model.status, author$project$Types$RemoveBoxBoxSelection)) {
																			return _List_fromArray(
																				[
																					...
																					_List_fromArray(
																						[
																							elm$html$Html$text('Choose box which you want to remove.')
																						]))
																				]);
																		} else {
...
var author$project$View$view = function (model) {
	var odlModal = function () {
		...
		return _List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class(classValue),
						A2(elm$html$Html$Attributes$attribute, 'aria-hidden', 'false'),
						elm$html$Html$Attributes$id('view_odl_overlay')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								...
								_List_fromArray(
									[
										elm$html$Html$text('Content (ODL):')
									])),
								A2(
								elm$html$Html$div,
								_List_fromArray(
	...
	var formContent = _List_fromArray(
		[
			A2(
			elm$html$Html$input,
			_List_fromArray(
		...
		return _List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$Attributes$class(classValue),
						elm$html$Html$Attributes$id('edit_box_overlay')
					]),
				_List_fromArray(
					[
						A2(
			...
			[
				...
				_Utils_ap(
					...
					_Utils_ap(
						...
						_Utils_ap(
							importModal,
							_Utils_ap(odlModal, editBoxModal))))),
				A2(
				elm$html$Html$div,
				_List_fromArray(
					[
...
require("../sass/edit.sass");

var _Main = require("../../document_editor/src/Main.elm");
...
app.ports.overlay.subscribe(function (enable) {
  if (enable == true) document.querySelector("html").classList.add("noscroll");else document.querySelector("html").classList.remove("noscroll");
});
app.ports.setupTextEditor.subscribe(function (value) {
  var view_odl_overlay = document.getElementById("view_odl_overlay");
  var odl_editor_element;
  if (view_odl_overlay.classList.contains("invisible")) odl_editor_element = document.querySelector("#edit_box_overlay #odl_editor");else odl_editor_element = document.querySelector("#view_odl_overlay #odl_editor");
  var editor = ace.edit(odl_editor_element);
...
function adjust_spacing() {
  ...
  if (window_width >= 769) playground.style.marginTop = menu_height + "px";else playground.style.marginTop = "0px";
}

document.addEventListener("DOMContentLoaded", adjust_spacing);
window.addEventListener('resize', adjust_spacing);
},{"../sass/edit.sass":"../sass/edit.sass","../../document_editor/src/Main.elm":"../../document_editor/src/Main.elm"}],"../../../../.nvm/versions/node/v11.0.0/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
},{}]},{},["../../../../.nvm/versions/node/v11.0.0/lib/node_modules/parcel-bundler/src/builtins/hmr-runtime.js","edit.js"], null)
