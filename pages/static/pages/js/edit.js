		...
		switch (msg.$) {
			...
			case 'KeyInteraction':
				...
				var newModel = _Utils_eq(model.status, author$project$Types$Default) ? ((key === 's') ? _Utils_update(
					...
					model,
					{status: author$project$Types$DuplicateBoxChooseBox}) : model))))) : ((key === 'Escape') ? (_Utils_eq(model.status, author$project$Types$EditBox) ? (elm$core$List$isEmpty(model.documentDraft) ? _Utils_update(
					model,
					{selectedBoxId: 0, status: author$project$Types$Default}) : _Utils_update(
					model,
					{status: author$project$Types$EditBoxWarnUnsavedDraft})) : (_Utils_eq(model.status, author$project$Types$EditBoxWarnUnsavedDraft) ? _Utils_update(
					model,
					{documentDraft: _List_Nil, selectedBoxId: 0, status: author$project$Types$Default}) : _Utils_update(
					model,
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$text('You have unsaved draft. Apply the changes or press escape again to discard the changes.')
							]))
					])) : A2(
				elm$html$Html$div,
				_List_fromArray(
...
var author$project$View$view = function (model) {
	...
	var editBoxModal = (_Utils_eq(model.status, author$project$Types$EditBox) || _Utils_eq(model.status, author$project$Types$EditBoxWarnUnsavedDraft)) ? _List_fromArray(
		[
			A2(
			elm$html$Html$div,
			_List_fromArray(
